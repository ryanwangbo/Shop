...
var validatePrompt = {
    username:{
        ...
        isNull:"请输入用户名",
        error:{
            beUsed:"该用户名已被使用，请使用其它用户名注册，如果您是&quot;{1}&quot;，请<a href='http://localhost:8087/Shop/user/login.jsp' class='flk13'>登录</a>",
        ...
        if (!length && format) {
            ...
            if (!namestate || nameold != option.value) {
                if (nameold != option.value) {
                    ...
                    //校验用户名是否已使用
                    //${pageContext.request.ContextPath}/Shop/RegitsterAction&uid=" + escape(option.value) + "&r=" + Math.random(), function (date)
                    $.getJSON("AjaxService.jsp?action=CheckUnicknme&uid=" + escape(option.value) + "&r=" + Math.random(), function (date) {
                        if (date.success == 0) {
